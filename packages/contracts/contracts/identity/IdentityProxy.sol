...
contract IdentityProxy is ERC725 {
    ...
    constructor(address _owner, address _marketplaceAddress) public {
        owner = _owner;
    ...
    function getHash(address signer, uint value, bytes data) 
        public view 
        returns(bytes32) 
    {
        return keccak256(abi.encodePacked(signer, marketplaceAddress, value, data, nonce[signer]));
    }

    // original forward function copied from https://github.com/uport-project/uport-identity/blob/develop/contracts/Proxy.sol
    function forward(bytes sign, address signer, bytes data) public {
        uint value = 0 ether;

        // the hash contains all of the information about the meta transaction to be called
        bytes32 _hash = getHash(signer, value, data);
        ...
        // execute the call
        require(executeCall(marketplaceAddress, value, data), "Cannot execute the call");
        emit Forwarded(sign, signer, marketplaceAddress, value, data, _hash);
    }
